<?php
/**
 * Modified version of fast.php that uses the custom trained Tesseract model
 */
...
$useCustomModel = file_exists($customModel);
if ($useCustomModel) {
    echo "Using custom trained model: $customModel\n";
    $modelDir = dirname($customModel);
} else {
    echo "Custom model not found. Using default Tesseract model.\n";
}
        ...
        if (!file_exists($rawFile)) {
            ...
            if (false !== $pos) {
                ...
                while (false === $ansDone) {
                    ...
                    if ($useCustomModel) {
                        exec("/usr/bin/tesseract {$imgFile} " . __DIR__ . "/qq -l captcha --tessdata-dir \"{$modelDir}\" --psm 8 -c load_system_dawg=0 -c load_freq_dawg=0");
                    } else {
